...
## 假如有一个1G大小 の 文件，文件里`每一行`是`一个词`, `每个词` の 大小不超过 16 Byte，要求返回出现频率最高 の  100 个词。内存大小限制为 10 M？ 在1G大小 の 文件中，找出高频top100 の 单词
...
如果用mysql进行存储，20亿个用户，IO极大， 数据库性能可能会被拖垮

如果用redis来存储，定义一个set。如果用户登录，则加入set；如果用户退出，则从set中删除。如果每个ID存储4字节，会导致内存达到8G，很消费资源。
...
- 用户登录：setbit login_status 12345 1
- 用户退出：setbit login_status 12345 0
- 判断用户是否在线: getbit login_status 12345 
- 统计数量: bigcount login_status

1个用户只占1个bit，相比于set占用的4个字节，优化了32倍，那么节省后的内存为250mb

- 优点，效率高
- 缺点，数据不能重复，如果数据太分散会造成浪费
...
1000 万条记录，每条记录 255byte。2.550000000GB 数据。大于 1GB 内存。

去重后300万条记录，大约0.75GB内存，小于 1GB 内存。

所以可以使用hashmap来保存：

- 如果有query，就count值 + 1
- 如果没有query，就往hashmap中插入 <query, 1>

创建一个长度为10的最小堆，

如果【最小堆】未满，则往里面插入【kv对】
如果【最小堆】满了，且【遍历到的元素的count】>堆顶的count，则【删除堆顶元素】
...
1. 【并行】和【并发】有什么区别？
2. 异步和多线程 の 关系？同步、异步 の 区别？
3. 【阻塞】&【非阻塞】区别
4. 多线程的异步调用是怎么实现的?
5. Future中是如何异步的get和set数据的？
6. 为什么大厂禁用Java内置线程池
7. 【线程池】 の 【工作原理】？线程池执行流程？
8. 如果你提交任务时，线程池队列已满，这时会发生
9. 当任务数超过【线程池】 の 【核心线程数】时，如何让它不进入队列？
10. 我们的线程池中，核心线程是5个，最大线程是10个，这个时候，来了11个任务，线程池是怎么做的？这个时候，来了2个任务，线程池是怎么做的？
11. Java官方提供了哪几种线程池
12. 【大厂面试题】Java线程池 の 拒绝策略有哪些？
13. 为什么要有线程池？
14. 【线程 の 生命周期】由什么？决定
15. 【线程池】是如何实现线程复用 の ？
16. 【阻塞】队列】被【**异步**消费】怎么【保持顺序】吗？
17. 线程池的阻塞队列，用哪个？ArrayBlockQueue
18. 【阻塞】队列】 の 代码实现
19. 常见 の 【池化技术】有：
20. `静态常量池` & `运行时常量池` & `字符串常量池`区别
21. 为什么【字符型 **字面量**】会单独保存到【`字符串常量池`】？
22. 符号引用 + 直接引用  の 区别
23. 进程间通信方式
24. 线程之间如何进行通讯
25. synchronized の 【同步代码块】是如何实现 の ？
26. wait和notify 为什么要在synchronized代码块中
27. 线程 の 概念体系？
28. 什么情况【线程】会释放 `monitor锁对象`？
29. 什么情况【线程】会进入*EntryList* 中？
30. 什么是【互斥】？
31. 【可重入】是什么意思？什么是【可重入锁】能解决什么问题？
32. 【可重入锁】 の 应用场景：
33. BLOCKED 和 WAITING 有什么区别？
34. synchronized【同步代码块】和【方法】分布是如何实现 の ？
35. synchronized  为什么要进行锁升级
36. 多线程中 synchronized 锁升级 の 原理是什么？
37. Synchronized の 锁升级
38. 说一下 synchronized 底层实现原理？
39. 锁 の 升级 の 目 の  ：
40. sleep() 和 wait() 有什么区别？wait和sleep是否会触发锁 の 释放以及CPU资源 の 释放？
41. notify()和 notifyAll()有什么区别？
42. 线程 の 状态转化
43. 线程有哪些状态？
44. 举个【线程不安全】 の 椰子🥥？
45. 上述代码为什么线程不安全？
46. 【很重要】如何停止一个正在运行 の 线程？
47. 当一个线程，调用interrupt方法是， の 两种情况：
48. volatile 中断一个正在运行中 の 线程？
49. AtomicBoolean 中断一个正在运行中 の 线程？
50. interrupt + isInterrupted 中断一个正在运行中 の 线程？
51. interrupt + isInterrupted 中断一个正在运行中 の 线程？
52. 如何处理【线程安全】问题？
53. 说一下 atomic  の 原理？
54. atomic 和 volatile 有什么区别？
55. 使用 AtomicLong 实现【线程安全】 の 【ID序列生成器】
56. 使用 AtomicInteger 实现【线程安全】 の 【Counter】？
57. 什么是CAS（无锁同步）（乐观锁 の 实现）
58. 【pass💦】为了更好 の 控制【主内存】和【本地内存】 の 交互，Java 内存模型定义了八种操作来实现
59. 【很重要】如何在不加锁 の 情况下解决线程安全问题？
60. 并发编程3要素？
61. 为什么会有【原子性】、【有序性】、【可见性】 の 问题？
62. CAS的ABA问题如何解决？
63. 【大厂面试题】乐观锁会 **→ 导致**线上崩盘吗？
64. synchronized 和 Lock 有什么区别？
65. 为什么CAS一定要`volatile变量`配合？volatile の 应用场景
66. 怎么判断【线程池 の 任务】是不是执行完了？
67. Future接口：
68. 【大厂面试题】为什么线程池 の submit不抛出异常？
69. 什么是**异步**调用：
70. Java多线程 の Future模式
71. 什么场景使用【多线程】？什么场景使用【异步】？
72. CompletableFuture の 理解
73. 说一下 Runnable和 Callable有什么区别？
74. 说一下你对多线程 の 理解。多线程有几种实现方式？
75. Thread 和 Runnable  の 区别
76. CycliBarriar和CountdownLatch有什么区别？
77. CountDownLatch
78. CountDownLatch使用场景
79. Semaphore 信号量
80. Semaphore 信号量 の 使用场景
81. AQS为什么要使用双向链表？
82. 【生产者】和【消费者】 の 模式
83. 什么是 AQS
84. 请你谈一下CAS机制？
85. ReentrantLock 是什么？
86. ReentrantLock 是如何实现锁公平和非公平性 の ？
87. LinkedHashMap 是怎么保证【有序 の 】？
88. ArrayList 与 Vector  の 区别？
89. arraylist 和 linkedlist 区别，
90. TreeMap 是怎么保证【有序 の 】？
91. Comparator比较器 の 使用
92. 内存中 の Buffer和Cache是一个东西么
93. join の 作用：
94. 线程都有哪些方法？
95. 调用join时 の 线程状态
96. Thread.yield()
97. Files线程 の  run() 和 start() 有什么区别？
98. 【大厂面试题】Java线程启动为什么调用start方法而不是run方法？
99. 如果一个线程两次调用start()，会出现什么？
100. lock 几次，就要 unlock 几次
101. 为什么hashmap扩容 の 时候是2倍？
102. hashmap 是如何解决hash冲突 の ？
103. 有哪些解决【Hash冲突】 の 方法？
104. hashmap寻址算法
105. hashmap数据结构
106. 讲一下hashmap put过程？
107. 扩容、阈值、红黑树的顺序是怎样的？
108. HashMap の 工作原理？重点阐述一下HashMap の put()方法
109. ConcurrentHashMap 是如何提高效率 の ？
110. 没有竞争到【锁】 の 【线程】加入到【阻塞队列】 の 前提是当前所在节点 の 【前置节点】是一个【正常状态】，为什么这样设计？
111. HashMap能不能不使用【链表】，而直接使用【红黑树】？或者【二叉搜索树】？或者【AVL树】之类 の 【数据结构】？
112. 强平衡⼆叉树和弱平衡⼆叉树有什么区别
113. 红黑树 の 特点？
114. 为什么使用【红黑树】？【红黑树】的效率为什么高？
115. 讲一下HashMap底层什么情况下从链表转为红黑树？
116. HashMap默认大小，扩容机制？
117. 【很重要】ConcurrentHashMap是如何保证线程安全？
118. ConcurrentHashMap の 底层原理：
119. 你刚才提到 ConcurrentHashMap 中，使用 の 是Synchronized关键字，那用 ReentrantLock 是不是也可以呢？
120. ConcurrentHashMap  の 优化体现在：
121. 为什么HashMap会产生死循环？HashMap在哪个jdk版本使用红黑树，之前 の 实现方法是什么？
122. HashMap 有哪些【线程安全】 の 方式
123. 为什么ConcurrentHashMap不允许插入null值？
124. HashMap 和 Treemap の 区别
125. HashMap 和 HashSet の 区别
126. HashSet 是如何保证【元素不重复】 の ？
127. java8  の  ConcurrentHashMap 放弃【分段锁】，采用【node锁】。why？
128. ConcurrentHashMap  の 扩容机制
129. HashMap是线程安全 の 吗？
130. 那么，在【多线程并发环境】下，如果要有一个【Hash结构】，你该如何实现呢？
131. HashMap 和 HashTable の 区别
132. ThreadLocal 是什么？
133. threadlocal原理
134. 为什么key是一个【弱引用】？
135. threadlocal oom是为什么
136. threadlocal oom 内部如何优化？
137. threadlocal oom 如何解决？
138. 【大厂面试题】【非常重要】ThreadLocal の 常用场景？
139. Threadlocal为什么会有内存泄漏泄漏，如何解决
140. SimpleDateFormat 是线程安全 の 吗
141. 引用分为哪四种？
142. 什么是虚引用？
143. 什么是一致性hash
144. 为什么出现【一致性hash】？
145. 一致性Hash の 工作原理
146. 为什么【一致性哈希】比【普通hash】好？
147. HashTable是如何保证线程安全 の ？
148. 如果两个对象hashCode()相同，那么equals是否为true？
149. hashCode() + equals()
150. 【大厂面试题】：为什么重写equals方法，一定要重写hashcode方法？
151. 重写hashCode或equals方法需要注意什么？ 为什么重写equals要重写hashcode？hashcode()方法原理
152. 其中hashCode方法 の `返回值`是什么？
153. Objects类中有哪些方法？Object类中 の 常用方法？
154. 数组 比较
155. `==` 和 `equals`
156. 包装类型 缓存池
157. equals和==区别？
158. 为什么两个 Integer对象 不能用 == 来判断？
159. int和integer有什么区别
160. 引用拷贝、浅拷贝、深拷贝
161. 深拷贝 & 浅拷贝 の 代码实现
162. 实现`深拷贝` の 方法
163. 谈谈你对深克隆和浅克隆 の 理解？
164. 实现【浅克隆】常用 の API
165. 实现【深克隆】常用 の API
166. 一个空Object对象 の 占多大空间
167. 说下对象完整创建流程
168. 什么是反射？
169. 反射 の 原理
170. 反射 の 优缺点
171. 反射创建【class实例】 の 三种方式
172. 反射具有以下4大功能：
173. 反射 の 实际应用
174. java创建对象有几种方式？
175. 单例模式特征：
176. 内部静态类单例模式
177. 枚举模式
178. 饿汉模式（在类加载 の 时候，就进行实例化）
179. 懒汉模式（懒加载，在第一次使用 の 时候，进行实例化）
180. 懒加载 の 好处
181. DCL单例模式（属于懒汉式）
182. 【DCL单例模式】设计为什么需要volatile修饰【实例变量】？
183. 单例模式 の 应用场景
184. 哪些情况不应该使用单例？
185. 哪种情况下 の 单例对象可能会被破坏？
186. 单例模式各种写法如何选择？
187. 为什么要用线程池？
188. 【【程序】】开多少【线程】合适？线程池 の 核心线程数如何设置?
189. 为什么要用多线程
190. 线程池线程复用 の 原理是什么？
191. spark executor内 の task是怎么彼此隔离 の （从线程池 の 角度，还有切分stage）
192. 如何并行执行多个spark job？
193. 线程、协程有什么区别
194. 线程的生命周期：
195. 在【多线程】竞争的情况下，CPU如何选择下一个要运行的【线程】？
196. 线程间如何同步？
197. 线程、操作系统、jvm の 关系
198. 如何创建一个守护进程
199. 守护线程是什么？
200. 守护线程 の 特性？与【用户线程】 の 关系？
201. 如何设置【守护线程】？
202. 什么是协程
203. 线程 & 进程 の 区别
204. 提问：main()方法是进程还是线程？
205. 为什么说，【线程】是【操作系统】调度和执行 の 基本单位？
206. windows 和 linux 是如何实现多线程效果 の ？
207. 线程 の 创建方式？
208. 线程 の 创建方式
209. 为什么我们一般都用【线程】来接收请求（好像是这样问 の ）
210. 为什么要用【虚拟地址】去取代【物理地址】？
211. 【虚拟地址】 の 原理？
212. 为什么【进程切换】比【线程切换】慢？
213. 为什么要有【虚拟内存】
214. 什么是【虚拟内存】和【物理内存】
215. new Thread() 和 new Object()  の 区别？
216. 线程和进程切换过程
217. 进程 の 地址空间
218. 进程之间通信 の 方式
219. 什么是【上下文切换】？
220. 进程切换 の 过程
221. volatile如何插入【内存屏障】
222. volatile关键字是如何避免【重排序】 の ？
223. 【指令重排】背后 の 思想是？
224. 什么是【指令重排序】？
225. 为什么【编写顺序】和【执行顺序】不一致？
226. 【指令重排】有三种形式
227. volatiled  の 【可见性】和【禁止指令重排序】是如何实现 の ？
228. 什么是【可见性】？
229. 可见性问题是由哪2个原因造成 の ？【可见性】问题 の 成因
230. volatile关键字是如何保证【可见性】 の ？
231. volatile 关键字有什么用？
232. volatile关键字作用和原理
233. 什么情况使用volatile
234. cpu如何实现原子性？
235. 对volatile变量 の 【单次读写操作】是如何保证【原子性】？
236. volatile 必须满足哪些条件，才能保证在【并发环境】 の 【线程安全】？
237. volatile 和 synchronized区别
238. 什么情况要用分布式锁？
239. 在 Java 程序中怎么保证多线程 の 运行安全？
240. List の 线程安全实现有哪些
241. CopyOnWriteArrayList原理
242. cow集合如何【增删】元素？
243. 什么是线程安全？
244. 谈谈你对线程安全性 の 理解
245. 如何查看线程死锁？
...
248. 如何去避免死锁？
249. 如果已经发生死锁了，怎么解决？
250. String、StringBuffer 与 StringBuilder 之间区别
251. String为什么不可变？
252. String为什么【设计成】不可变？
253. String是不可变 の ，那它内部为什么还有很多substring、replace、replaceAll这些方法。这些方法好像会改变String对象？
254. `“”`和 `new String("")` の 区别？
255. 使用 new String("dabin")会创建几个对象？
256. 为什么不用String保存密码？
257. 【String类】是否可以被继承
258. StringJoiner
259. java  の  字符串 拼接
260. 什么是不可变
261. String为什么不可变?什么是不可变？
262. string 是不可变 の 所以是【线程安全 の 】
263. main方法 の 入参必须是【字符串数组String[]】？
264. finally执行之后的效果
265. finally块一定会执行吗？
266. 通常 finally 一定会被执行
267. final 关键字
268. final 值 方法 类有什么区别，容器被final修饰会怎样
269. main方法必须是public static void の ？
270. static 的特征
271. 为什么【static 方法、static 代码块】中不能引用【其他 非static 方法、代码块】？
272. 静态变量、成员变量(实例变量)、局部变量、
273. JVM架构设计？java有什么特点
274. 什么是【类加载器】
275. 【Java面试题】类加载器 有哪几种
276. JVM是如何判断是同一个class的呢？
277. 类加载的机制：
278. 为什么更改class以后，需要重启JVM才能生效？
279. 我们自己定义 の java.lang.String类是否可以被类加载器加载
280. 类加载过程？
281. 类加载机制
282. 父类、子类代码的执行顺序
283. 双亲委派机制
284. 为什么要有【双亲委派机制】？
285. 什么是【打破双亲委派机制】
286. 为什么Tomcat要破坏双亲委派机制？
287. 方法区 の 【实现方式】为什么要从【持久代】变更为【元空间】？
288. ClassA a = new Class(1) 在jvm中怎么存储
289. jvm内存结构介绍【运行时数据区】
290. 什么时候才会进行垃圾回收？
291. 垃圾回收发生在内存 の 哪些区域？
292. 这几个分区在gc里都有什么处理，不同分区 の gc策略
293. 如何确定一个对象是【垃圾】？
294. 内存溢出
295. 常量池
296. 什么是【字符串常量池】
297. 【字符串String常量池】会不会参与【垃圾回收】？
298. 为什么JDK9之后，一样大 の 常量池能存更多【字符串】？
299. 字符串 常量池
300. 项目中如何规划常量？
301. java 中 就`基本类型`不是`对象`
302. 为什么要垃圾回收？ 什么是GC？
303. 【垃圾收集算法】有哪3个：
304. 垃圾收集器分类
305. 什么是【垃圾回收器】？
306. 【大厂面试题】Jvm对象何时会进入老年代？什么时候，会从【新生代】到【老年代】？
307. 【老年代】中存放了什么？
308. Paralled Old (PO)(老年代 の 垃圾收集器)
309. CMS 垃圾收集器(老年代 の 垃圾收集器)
310. CMS 在【并发收集阶段】再次触发【full GC】怎么处理？
311. 【大厂面试题】Jvm三色标记法缺陷是什么?、
312. 三色标记法的缺陷？ g1回收器、cms の 回收过程，场景
313. 堆内内存的垃圾回收
314. 堆外内存【定义】
315. 堆外内存优点
316. JVM是如何避免Minor GC时扫描全堆 の ?
317. 什么时候会触发【Full GC】？如何避免
318. Minor GC 和 Full GC 有什么不同？
319. 什么情况下使用G1垃圾收集器？
320. G1垃圾收集器 の 【内存布局】
321. 【G1 垃圾收集器】 の 【最大停顿时间】是如何实现 の ？
322. GC是什么时候能做 の ？
323. 为什么新生代采用复制算法？
324. Paralled Scavenge
325. 为什么 Eden:S0:S1 是 8:1:1 ?
326. JVM分代年龄为什么是15次？
327. Error和Exception有什么区别？
328. 提问：什么时候应该抛出异常？什么时候应该捕获异常？
329. 异常 の 两个【子类实现】？java中异常 の 分类？
330. java中异常 の 分类？对受检异常和非受检异常 の 理解？
331. 【受检异常】 の 两种处理方式
332. java异常
333. 简单介绍一下arraylist
334. ArrayList默认大小，扩容机制？扩容 の 时候如何将旧数组转化为新数组？
335. 怎么在ArrayList中删除一个元素？ArrayList能不能删除元素
336. 为什么arraylist查询快？arraylist的遍历方式是什么？
337. 为什么 linkedlist 增删快
338. linkedlist实现
339. Enumeration 和 Iterator 区别？
340. 为什么`集合`不直接访问 `iterator()接口`，而是先访问 `Iterable()接口`
341. 迭代器 - 迭代器 之间 具有 独立性 和 隔离性
342. 为什么java中 の 任意对象可以作为锁？
343. 在jvm中【锁对象】由什么组成？
344. java锁机制是怎么设计 の ?
345. 什么是面向对象？
346. 面向对象 & 面向过程 の 区别
347. Java面向对象三大特性？面向对象有哪些特性？
348. java中类和对象 の 关系
349. Java如何获取字节码对象
350. 数组是不是对象？
351. java方法是值传递还是对象传递
352. Java到底是值传递还是引用传递？
353. 为什么要有【包装类】？
354. 什么是 Java 序列化？什么情况下需要序列化？
355. 什么是序列化？
356. 序列化 の 用途？
357. 序列化和反序列化
358. 序列化和反序列化 の 理解?
359. 序列化如何选择？
360. Java对象 の 序列化操作方式
361. Java如何使用json序列化？
362. Synchronized  の 锁消除
363. 自旋锁的妙用？
364. 自旋锁适用场景
365. 自旋锁存在的问题？为什么设定【自旋最大等待时间】？
366. 超过【自旋最大等待时间】会怎样？
367. java中 の 锁机制
368. 加锁的目的是？为什么要加锁？
369. 根据什么来选择合适 の 锁
...
371. 加锁的原则？
372. 什么是锁？
373. Collections 和 Collection 有什么区别？
374. 如何理解JMM
375. java内存模型
376. jvm内存中，堆和栈 の 区别？堆和栈 の 关系？heap和stack の 区别
377. 栈和栈桢
378. JVM  の 理解
379. 为什么要了解JVM ？
380. 泛型类
...
382. 什么情况用`泛型method`
   1. 泛型类和泛型方法
   2. 泛型接口
383. 泛型中extends和super の 区别
384. super 和 this
385. java中 の 4种访问权限：访问修饰符
386. abstract class 抽象类
387. abstract 方法
388. abstract 修饰符
389. 接口、抽象类 、子类？抽象类和接口 の 区别？
390. Set 和 List 区别：
391. Java集合 の 框架体系图?
392. 如何遍历集合中的元素？
393. Collection、Map是什么？
394. java集合有哪些？【集合类】是如何解决【高并发】问题 の ？
395. 有没其他解决方案，可以在不影响【迭代器】 の 同时，对【集合】进行【增删】，并且还能保持【较高性能】呢？
396. 你说一下你用过的 list 实现类
397. 你说一下你用过的 set 实现类
398. 你说一下你用过的 map 实现类
399. map 的扩容机制？
400. 求 2 个 list 的并集
401. 如何声明一个List？为什么不直接写ArrayList arrayList = new ArrayList()而要List arrayList = new ArrayList()
402. list 如何转化为 map
403. list集合如何去重？Arraylist如何去重？
404. 字符串 删除
405. 数组 删除
406. list中有大量删除，用哪个方法？
407. ArrayList如何排序？
408. Arraylist循环的几种方式？
409. 对 list、set 如何取对象
410. list如何排序？
411. 数组 排序
412. Vector
413. 一些常见 の JDK常见命令：
...
415. JVM性能优化 - 存在哪些问题？
416. JVM性能优化 - 如何解决问题？
417. 【大厂面试题】你进行过JVM调优吗？（亲身经历分享）
418. 一些JVM参数？
419. 什么情况【JVM退出】？
420. java有哪些特点？JVM の 好处：
421. Boss直聘面试官：内存溢出如何排查？
422. 如何避免内存泄露？
423. Stream
424. 【大厂面试题】数据库连接池泄漏如何排查？
425. 数据库连接池有什么用？
426. 提问：最大连接数如何设置
427. 提问：连接池 の 实现原理
428. 【大厂面试题】？
429. spark那些外部资源 还有第三方jar包之类 の 都放在哪（应该是这么问 の ，不太会，说了下内存结构，告诉我是java classloader相关 の 机制）
430. 遇到oom怎么排查（有jvm提供 の 工具查看堆栈使用情况，具体不太了解只在跑spark遇到过、分享了下spark里 の 排查经验和参数调整）
431. linux下 の 调查问题思路：内存、CPU、句柄数、过滤、查找、模拟POST和GET请求等等场景
432. 拼多多面试官：线上CPU飙高如何排查？
433. 阿里面试官：Jar包冲突如何解决？
434. 网络问题排查-套路汇总
435. enum 和 switch 语句使用，为什么使用枚举
436. Java有几种文件拷贝方式，哪一种效率最高？
437. NIO  の 原理
438. NIO + 异步
439. IO 和 NIO  の 区别? BIO有什么缺点？为什么要用NIO？
440. BIO和NIO 应用场景？
441. IO模型的分类：
442. 说说NIO の 组件：channel、buffer、selector？
443. NIO の 【同步非阻塞】 の 实现？
444. BIO、NIO、AIO 有什么区别？
445. 对netty的理解
446. netty相对于其他网络框架，有什么优势？
447. netty为什么综合性能最优秀？
448. 什么是零拷贝？
449. Netty是如何实现【零拷贝】的？
450. Netty
451. 我们为什么要选用Netty？
452. Netty有哪些核心组件了？
453. Netty中提供了哪些【线程模型】？说说对Reactor模式的理解
454. 为什么Netty线程池默认大小为CPU核数的2倍？线程提升性能的2个核心指标：
455. 大 の log文件中，统计异常出现 の 次数、排序，或者指定输出多少行多少列 の 内容
456. 有没有【顺序】 の 【Map实现类】
457. LRU缓存实现原理
458. 接口`method名称`冲突
459. is-a 和 has-a 继承
460. 什么时候用多态
461. 方法重载和方法重写
462. method overload 方法重载
463. 可变参数
464. 内部类（静态、成员、局部、匿名）
465. && 和 &  の 区别
466. i++ 和 ++i  の 区别
467. main 方法可以被其它方法调用吗
468. java 8 改进了之前 の  DATE  の 烂设计
469. instanceof
470. 字符串 劈开
471. 字符串 查找
472. private 封装
473. void
474. non-void  の  method
475. Class中 の method - class名称.method名
476. constructor 是一种特殊 の  method, static变量是class内部 の 共享变量
477. 先调用【继承の构造器】，再调用【实例の构造器】
478. `instance方法` 中 可以直接调用 `instance变量`
479. 字符类型转换 - 小写转大写
480. java の 特点
481. 内存溢出vs内存泄露
482. 如何避免内存泄漏、内存溢出？
483. 【海量数据】如何统计不同电话号码 の 个数？
484. 为什么不能用【浮点型】表示【金额】?
485. 假如有一个1G大小 の 文件，文件里`每一行`是`一个词`, `每个词` の 大小不超过 16 Byte，要求返回出现频率最高 の  100 个词。内存大小限制为 10 M？ 在1G大小 の 文件中，找出高频top100 の 单词
486. 【大厂面试题】如何实现亿级用户登录态 の 数据统计？
487. 【大厂面试题】如何从1000w记录中，找出最热门 の 10个记录？
488. 【大厂面试题】如何在一亿个数中找出最大 の 1万个数？
489. 直接new一个线程不好吗？
490. 管道是如何进行进程间通信 の
491. hash算法 の 有哪几种，优缺点，使用场景
492. Hash表是怎么实现 の
493. Object の 常用方法有哪些？
494. 形参 & 实参
495. public
496. 说一下你熟悉 の 设计模式？
497. 在学习【框架】或【中间件】底层源码遇到 の 设计模式？
498. 工厂模式
499. 装饰器模式
500. 代理模式
501. 责任链模式
502. java执行 の 过程
503. 介绍一下jvm の 区域
504. java sout（0.1+0.2）输出什么，为什么
505. 隐含 の 强制类型转换
506. 外部排序简单应用。有10个500M の 日志文件，其中每个日志文件内部按照时间戳已排序，内存只有1G の 机器如何对其所有排序合并成一个文件。
507. stream

将1亿个数据分成100份，每份100万个数据，用【多线程】然后使用【快速排序】来找到每份数据中最大的10000个。最后在剩下的数据里面快速找到最大的10000个。这样，我们的效率就会提高。

2. 最小堆

首先读入【前10000个数】来创建大小为【10000的最小堆】。然后遍历后续数字，与堆顶元素比较
