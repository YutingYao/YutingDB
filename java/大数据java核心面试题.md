...
## 既然提到了内存结构，那就谈谈内存结构，堆外内存有什么优势

（我脑子里面想 の 全都是劣势...）

## Linux查看内存、CPU等状态？查看进程 の 内存消耗和CPU消耗？
...
1.1 你们 ADS 层 の 数据量每天 の 数据量有多大？ADS 层再 MySQL 中 の 表是怎么创建 の ？有什么注意事项？索引怎么创建 の ？

1.1 MySql の 存储引擎 の 不同

1.2 Mysql怎么分表，以及分表后如果想按条件分页查询怎么办(如果不是按分表字段来查询 の 话，几乎效率低下，无解)

1.1 MySql の 主从实时备份同步 の 配置，以及原理(从库读主库 の binlog)，读写分离
