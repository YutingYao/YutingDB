...
- 写时复制。当add新元素时，需要copy一份数据，然后将【原引用】指向【new数组】
- add方法：需要加锁
- 读操作：不需要加锁
...
5. 线程池线程复用的原理是什么？
6. 线程池 の 工作原理
7. 如果你提交任务时，线程池队列已满，这时会发生
8. 怎么判断线程池的任务是不是执行完了？
9. volatile关键字作用和原理
10. 可见性问题是由哪2个原因造成的？
11. volatile如何插入【内存屏障】
12. 什么情况使用volatile
13. 如何停止一个正在运行的线程？
14. 线程池如何知道一个线程 の 任务已经执行完成？
15. CycliBarriar和CountdownLatch有什么区别？
16. 什么是锁？
17. 线程之间如何进行通讯
18. 线程池是如何实现线程复用 の ？
19. 【生产者】和【消费者】 の 模式
...
21. 单线程如何实现异步？
22. 阻塞队列被异步消费怎么保持顺序吗？
23. 当任务数超过【线程池】 の 【核心线程数】时，如何让它不进入队列？
24. 并发编程3要素？
25. 什么是 AQS
26. AQS为什么要使用双向链表？
27. 线程都有哪些方法？
28. 当一个线程，调用interrupt方法是，的两种情况：
29. 没有竞争到【锁】的【线程】加入到【阻塞队列】的前提是当前所在节点的【前置节点】是一个【正常状态】，为什么这样设计？
30. 【阻塞队列】的代码实现
31. 谈谈你对线程安全性的理解
32. 为什么会有【原子性】、【有序性】、【可见性】的问题？
33. 【很重要】如何在不加锁 の 情况下解决线程安全问题？
34. HashMap八连问
35. HashMap的工作原理：
36. 红黑树的特点？
37. 为什么使用【红黑树】？
38. HashMap能不能不使用【链表】，而直接使用【红黑树】？或者【二叉搜索树】？或者【AVL树】之类的【数据结构】？
39. HashMap是线程安全的吗？
40. 那么，在【多线程并发环境】下，如果要有一个【Hash结构】，你该如何实现呢？
41. ConcurrentHashMap的底层原理：
42. Synchronized的锁优化
43. 为什么hashmap扩容的时候是2倍？
44. 为什么HashMap会产生死循环？
45. HashMap 和 HashTable の 区别
46. HashTable是如何保证线程安全的？
47. String、StringBuffer 与 StringBuilder 之间区别
48. String为什么不可变？
49. String为什么【设计成】不可变？
50. String是不可变的，那它内部为什么还有很多substring、replace、replaceAll这些方法。这些方法好像会改变String对象？
51. 什么是【互斥】？
52. volatiled 的【可见性】和【禁止指令重排序】是如何实现的？
53. volatile关键字是如何实现【可见性】的？
54. volatile关键字是如何避免【重排序】的？
55. volatile 关键字有什么用？
56. 什么是【可见性】？
57. 【可见性】问题的成因
58. volatile关键字是如何保证【可见性】的？
59. 具体来讲，Volatile 保证可见性 の 原理
60. 对volatile变量 の 【单次读写操作】是如何保证【原子性】？
61. volatile 必须满足哪些条件，才能保证在【并发环境】 の 【线程安全】？
62. volatile 和 synchronized区别
63. cpu如何实现原子性？
64. 【指令重排】背后 の 思想是？
65. 什么是【指令重排序】？
66. 【指令重排】有三种形式
...
68. 内部静态类单例模式
69. 枚举模式
70. 饿汉模式（在类加载的时候，就进行实例化）
71. 懒汉模式（懒加载，在第一次使用的时候，进行实例化）
72. 懒加载的好处
73. DCL单例模式（属于懒汉式）
74. 【DCL单例模式】设计为什么需要volatile修饰【实例变量】？
75. 单例模式的应用场景
76. 哪些情况不应该使用单例？
77. 哪种情况下的单例对象可能会被破坏？
78. 单例模式各种写法如何选择？
79. 什么是CAS（无锁同步）（乐观锁的实现）
80. 为什么CAS一定要`volatile变量`配合？volatile的应用场景
81. synchronized
82. 线程 の 状态转化
83. 如果一个线程两次调用start()，会出现什么？
84. 调用join时的线程状态
85. BLOCKED 和 WAITING 有什么区别？
86. Thread.yield()
87. 描述一下Object类中的常用方法？
88. sleep() 和 wait() 有什么区别？
89. wait和sleep是否会触发锁 の 释放以及CPU资源 の 释放？
90. notify()和 notifyAll()有什么区别？
91. wait和notify 为什么要在synchronized代码块中
92. 说一下 synchronized 底层实现原理？
93. synchronized  为什么要进行锁升级
94. 线程有哪些状态？
95. 说一下 Runnable和 Callable有什么区别？
96. 说一下你对多线程 の 理解。多线程有几种实现方式？
97. Thread 和 Runnable  の 区别
98. 说一下 atomic  の 原理？
99. atomic 和 volatile 有什么区别？
100. 使用 AtomicLong 实现【线程安全】的【ID序列生成器】
101. 使用 AtomicInteger 实现【线程安全】的【Counter】？
102. Files线程 の  run() 和 start() 有什么区别？
103. join的作用：
104. CopyOnWriteArrayList原理
105. cow集合如何【增删】元素？
106. Java多线程 の Future模式
107. synchronized 和 Lock 有什么区别？
108. lock 几次，就要 unlock 几次
109. ReentrantLock 是什么？
110. ReentrantLock 是如何实现锁公平和非公平性 の ？
111. java集合有哪些？
112. 【集合类】是如何解决【高并发】问题的？
113. HashMap 有哪些【线程安全】 の 方式
114. 在 Java 程序中怎么保证多线程 の 运行安全？
115. 为什么ConcurrentHashMap不允许插入null值？
116. java8  の  ConcurrentHashMap 放弃【分段锁】，采用【node锁】。why？
117. ConcurrentHashMap  の 扩容机制
118. ConcurrentHashMap 是如何提高效率 の ？
119. ConcurrentHashMap是如何保证线程安全？
120. ConcurrentHashMap 的优化体现在：
121. 守护线程是什么？
122. 守护线程的特性？与【用户线程】的关系？
123. 如何设置【守护线程】？
124. 什么是协程
125. 线程、协程有什么区别
126. 线程 & 进程 の 区别
...
131. 如何理解JMM
132. java内存模型
133. 【pass💦】为了更好 の 控制【主内存】和【本地内存】 の 交互，Java 内存模型定义了八种操作来实现
134. jvm内存结构介绍【运行时数据区】
135. 方法区的【实现方式】为什么要从【持久代】变更为【元空间】？
136. 常量池
137. `“”`和 `new String("")`的区别？
138. 【字符创常量池】会不会参与【垃圾回收】？
139. 为什么JDK9之后，一样大的常量池能存更多【字符串】？
140. 为什么不用String保存密码？
141. 字符串 常量池
142. 项目中如何规划常量？
143. JVM分代年龄为什么是15次？
144. 栈和栈桢
145. ClassA a = new Class(1) 在jvm中怎么存储
146. JVM 的理解
147. 为什么要了解JVM ？
148. 类加载机制
149. 【Java面试题】类加载器
150. 为什么Tomcat要破坏双亲委派机制？
151. 双亲委派机制
152. JVM的运行流程？
153. 为什么要垃圾回收？
154. 【垃圾收集算法】有哪3个：
155. 垃圾收集器分类
156. 为什么新生代采用复制算法？
157. Paralled Scavenge
158. 什么情况下使用G1垃圾收集器？
159. Minor GC 和 Full GC 有什么不同？
160. G1垃圾收集器的【内存布局】
161. 【G1 垃圾收集器】的【最大停顿时间】是如何实现的？
162. Paralled Old (PO)(老年代的垃圾收集器)
163. CMS 垃圾收集器(老年代的垃圾收集器)
164. CMS 在【并发收集阶段】再次触发【full GC】怎么处理？
165. 什么是【垃圾回收器】？
166. 什么时候才会进行垃圾回收？
167. 什么时候，会从【新生代】到【老年代】？
168. 【老年代】中存放了什么？
169. 垃圾回收发生在内存的哪些区域？
170. 引用分为哪四种？
171. GC是什么时候能做的？
172. 如何确定一个对象是【垃圾】？
173. 为什么 Eden:S0:S1 是 8:1:1 ?
174. JVM是如何避免Minor GC时扫描全堆的?
175. g1回收器、cms の 回收过程，场景
176. 这几个分区在gc里都有什么处理，不同分区 の gc策略
177. 一些JVM参数？
178. JVM性能优化 - 存在哪些问题？
179. JVM性能优化 - 如何解决问题？
180. zk，redis 实现【分布式锁】，哪个更好？
181. 在jvm中【锁对象】由什么组成？
182. java锁机制是怎么设计的?
183. 为什么要用多线程
184. 多线程中 synchronized 锁升级 の 原理是什么？
185. 【可重入】是什么意思？
186. 锁 の 升级 の 目的 ：
187. jvm内存中，堆和栈 の 区别？堆和栈 の 关系
188. 堆内内存
189. 堆外内存【定义】
190. 堆外内存优点
191. Java虚拟机，问麻了！
192. enum 和 switch 语句使用，为什么使用枚举
193. 泛型中extends和super の 区别
194. 泛型类
195. 什么情况用`泛型class`？
196. 什么情况用`泛型method`
   1. 泛型类和泛型方法
...
200. 实现`深拷贝` の 方法
201. 谈谈你对深克隆和浅克隆的理解？
202. Java有几种文件拷贝方式，哪一种效率最高？
203. IO 和 NIO  の 区别?
204. BIO、NIO、AIO 有什么区别？
205. Error和Exception有什么区别？
206. 异常 の 两个【子类实现】？java中异常 の 分类？
207. java中异常 の 分类？对受检异常和非受检异常 の 理解？
208. finally 一定会被执行
209. 【受检异常】 の 两种处理方式
210. java异常
211. 大 の log文件中，统计异常出现 の 次数、排序，或者指定输出多少行多少列 の 内容
212. abstract class 抽象类
213. abstract 方法
214. abstract 修饰符
215. 抽象类和接口的区别？
216. 接口、抽象类 、子类
...
219. 如何去避免死锁？
220. 请你谈一下CAS机制？
221. 有没其他解决方案，可以在不影响【迭代器】 の 同时，对【集合】进行【增删】，并且还能保持【较高性能】呢？
222. 为什么`集合`不直接访问 `iterator()接口`，而是先访问 `Iterable()接口`
223. 迭代器 - 迭代器 之间 具有 独立性 和 隔离性
224. java中 の 锁机制
225. synchronized  の 作用
226. ArrayList默认大小，扩容机制？扩容 の 时候如何将旧数组转化为新数组？
227. HashMap默认大小，扩容机制？
228. 有没有【顺序】 の 【Map实现类】
229. LRU缓存实现原理
230. LinkedHashMap 是怎么保证【有序 の 】？
231. TreeMap 是怎么保证【有序 の 】？
232. Comparator比较器的使用
233. 接口`method名称`冲突
234. java 中 就`基本类型`不是`对象`
235. equals和==区别？
236. 为什么两个 Integer对象 不能用 == 来判断？
237. hashCode() + equals()
238. 重写hashCode或equals方法需要注意什么？ 为什么重写equals要重写hashcode？hashcode()方法原理
239. int和integer有什么区别
240. 数组 比较
241. `==` 和 `equals`
242. 包装类型 缓存池
243. is-a 和 has-a 继承
...
254. Java对象的序列化操作方式
255. Java如何使用json序列化？
256. 什么是 Java 序列化？什么情况下需要序列化？
257. 什么是序列化？
258. 序列化的用途？
259. 序列化和反序列化
260. java中类和对象 の 关系
261. main 方法可以被其它方法调用吗
262. final 关键字
263. java 8 改进了之前的 DATE 的烂设计
264. StringJoiner
265. java 的 字符串 拼接
266. 数组是不是对象？
267. Java到底是值传递还是引用传递？
268. string 是不可变的
269. instanceof
270. 数组 排序
271. 数组 删除
...
275. private 封装
276. void
277. non-void 的 method
278. Class中的method - class名称.method名
279. 为什么Java中的main方法必须是 public static void？
280. constructor 是一种特殊的 method, static变量是class内部的共享变量
281. 先调用【继承の构造器】，再调用【实例の构造器】
282. `instance方法` 中 可以直接调用 `instance变量`
283. 字符类型转换 - 小写转大写
284. java的特点
285. main方法必须是public static void的？
286. 在1G大小的文件中，找出高频top100的单词
287. 别再问我Java虚拟机八股文了！
288. java有哪些特点
289. 使用 `new String("dabin")` 会创建几个对象？
290. 内存溢出vs内存泄露
291. 【大厂面试】听说你精通Java？
292. 【大厂面试】字节跳动二面
293. 京东到家一面
294. 【海量数据】如何统计不同电话号码的个数？
| synchronized  |  lock |
|---|---|
| 是【方法关键字】  |  J.U.C包中的【接口】 |
...
| synchronized  |  lock |
|---|---|
| 无法实现【非阻塞竞争锁】  |  提供了【非阻塞竞争锁】tryLock()。可以通过返回【True.False】来告诉当前线程，是否已经有【其他线程】正在使用【锁】 |
...
https://www.bilibili.com/video/BV1KL411T7MT

https://www.bilibili.com/video/BV13i4y1y73C
...
https://www.bilibili.com/video/BV1FS4y1S7Ac
...
[NIO](https://www.bilibili.com/video/BV1zB4y127Gm)
...
https://www.bilibili.com/video/BV17L4y1N7LX
...
【扩容因子】代表了【hash冲突的概率】与【空间利用率】之间的 balance。根据【泊松分布】。在扩容因子为 0.75 时，链表长度达到 8 的可能性几乎为 0 。
...
https://www.bilibili.com/video/BV1XR4y1L7US
