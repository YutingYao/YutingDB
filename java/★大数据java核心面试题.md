...
1. put()方法
   - 在存储的时候，首先会调用一个【hash方法】，通过这个hash方法，可以计算出【Key的hash值】，从而得到一个【十进制的数字】，这个【数字】和【数组.length - 1】取模，就可以得到【数组的下标】，然后，我们可以根据【下标】去找到【数组中的单向链表】，将要插入的key进行【equals()】的比较，如果是相等的话，就直接更新value值，如果不相等的话，就把K-V值 put() 到【链表】中去。在put过程中，如果【哈希表】中存储的【键值对】超过了【数组长度 × 负载因子】，就会将这个【数组】扩容为2倍。还有，就是在【插入链表】的时候，如果【链表长度】超过了我们默认设置的阈值为8的时候，节点的数据结构，就会自动转化为【红黑树】。
...
2. 如果涉及到【线程的控制】，则可以使用【线程工具类】下面的【CountDownLatch | Semaphore】
3. 如果涉及到【线程的控制】，则可以使用【集合类】下面的【concurrent包】
4. 如果涉及到【线程的控制】，则可以使用【Synchronized关键词、ReentrantLock接口】
5. 如果涉及到【多机环境】，则可以使用【分布式锁】
